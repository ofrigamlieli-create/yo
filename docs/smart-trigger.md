...
### Feature overview
Smart Trigger is the first part of Kani. It checks whether selected text is valid for Kani before anything else happens.
...
- Sends one internal event (`kani:selection-qualified`) when the selection is valid.
...
2. Kani checks if the selection is empty/collapsed. If yes, it does nothing.
3. Kani counts selected words (minimum is 20, inclusive).
4. Kani checks exclusions (navigation and form/editable areas).
5. Kani finds the deepest valid content container around the selection.
6. If the selection qualifies, Kani dispatches `kani:selection-qualified` on `document`.
7. If it does not qualify, Kani stays silent.
...
- Event fires but payload missing:
  - Listen for `kani:selection-qualified` on `document`.
  - Read `e.detail.selectionText`, `e.detail.wordCount`, `e.detail.containerElement`.
- Same selection does not fire repeatedly:
  - This is expected. Kani deduplicates repeated identical qualified selections.
